...
class MeteorDetector:
    ...
    def __init__(self, 
        ...
        Args:
            min_length: 最小軌跡長度（像素）
            min_brightness: 最小平均亮度
            min_angle: 最小傾斜角度（排除水平線）
            hough_threshold: Hough 變換閾值
            canny_low: Canny 邊緣檢測低閾值
            canny_high: Canny 邊緣檢測高閾值
        """
    ...
    def detect(self, image, osd_bbox=(0, 460, 470, 80)):
        ...
        Args:
            image: BGR 彩色圖片
            osd_bbox: OSD 區域 (x, y, w, h)
        
        Returns:
            meteors: 偵測到的流星列表
            debug_info: debug 資訊（edges, mask 等）
        """
        # 預處理
...
使用範例：
  ...
  # 自訂 OSD 區域
  python script.py --folder ./images --osd-bbox 0,450,640,100
  ...
  python script.py --folder ./images --debug
    ...
    parser.add_argument('--min-length', type=int, default=20,
                       help='最小軌跡長度（像素），預設 20')
    parser.add_argument('--min-brightness', type=int, default=80,
                       help='最小亮度，預設 80')
