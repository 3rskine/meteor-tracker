...
# -*- coding: utf-8 -*-
"""
整合版流星偵測系統 - 結合 Kalman 濾波與增強驗證邏輯

主要特色:
1. Kalman 濾波器 - 時間感知的位置預測與平滑
2. Multi-frame 差分 - 適應不同 FPS
3. 靜止點過濾 - 排除不動的誤判
4. RANSAC 共線檢查 - 驗證短軌跡
5. 軌跡相似度分析 - 智能合併重複偵測
6. 動態閾值 - 自適應偵測敏感度
...
class MeteorTracker:
    def __init__(self, video_path, output_folder, debug=False):
        ...
        # Multi-frame 差分設定
        self.target_window_seconds = 0.2 #importantttttt
        self.gray_buffer = deque(maxlen=max(60, int(self.fps * 4)))
    ...
    # ==================== 亮點偵測 ====================
    def find_bright_spots(self, delta_frame, threshold=8):
    ...
    # ==================== 軌跡匹配（動態距離）====================
    def match_spot_to_track(self, spot, max_distance=180):
        best_track = None
        min_score = float('inf')
    ...
    def is_meteor_track(self, track):
        ...
        if len(points) == 0:
            return False
        if len(points) == 1:
            # 單幀候選：必須足夠亮且面積不大（避免大面積雜訊）
            single_b = points[0].get('brightness', 0.0)
            single_area = points[0].get('area', 999999)
            if single_b >= 35.0 and single_area < 200:
                return True
            else:
                if self.debug:
                    self.log(f"  [單幀過濾] Track {track['track_id']}: brightness={single_b:.1f}, area={single_area}")
                return False

        # 對於多幀（>=2）軌跡，執行原本的多重檢查（但放寬亮度門檻）
    ...
    def create_new_track(self, spot, frame_idx):
        ...
        kalman = SimpleKalman(spot['x'], spot['y'], init_vx=0.0, init_vy=0.0, 
                            process_var=15.0, meas_var=14.0)
        track = {
    ...
    # ==================== 即時合併機制 ====================
    def add_or_merge_meteor(self, new_meteor, time_gap_allow=10, 
                          angle_cos_threshold=0.92, overlap_threshold=0.1, 
                          brightness_corr_threshold=0.4):
        """嘗試合併相似軌跡"""
        best_idx = None
        best_score = -1.0
        ...
        if best_idx is not None:
            ...
            if self.debug:
                self.log(f"[合併] meteor_{best_idx} + 新軌跡 (score={best_score:.2f})")
            return True
    ...
    def process_video(self):
        """處理整個影片"""
        self.log("=" * 80)
        self.log("整合版流星偵測系統 - Kalman + 增強驗證")
        self.log("=" * 80)
